...
class Game:
    ...
    def get_all_possible_states_conv2d_hung(self, state_hung=None):
        ...
        gamestates, moves, add_scores, dones, is_include_hold, is_new_hold = self.get_all_possible_gamestates(state_hung)
...
class Agent:
    def __init__(self, turn):
        dir_path = os.path.dirname(os.path.realpath(__file__))
        weight_file_path = os.path.join(dir_path, 'outer_569')
        self.model = keras.models.load_model(weight_file_path)
    ...
    def get_actions(self, state):
        ...
        self.env_hung.current_state.define2(Tetromino_hung,piece_next_hungs,boolean_array)
        states, add_scores, dones, _, _, moves = self.env_hung.get_all_possible_states_conv2d()
        rewards = get_reward(add_scores, dones)
        q = rewards + self.model(states)
